...
ScoreTable GetScoredStudents(const Events& events, time_t score_time) {
    ...
    for (auto& event : events) {
        if (event.event_type == EventType::CheckFailed && event.time > table[event.student_name][event.task_name].cl_last_try && event.time <= score_time) {
            table[event.student_name][event.task_name].cl_last_try = event.time;
            table[event.student_name][event.task_name].cl_success = false;
        } else if (event.event_type == EventType::CheckSuccess && event.time > table[event.student_name][event.task_name].cl_last_try && event.time <= score_time) {
            table[event.student_name][event.task_name].cl_last_try = event.time;
            table[event.student_name][event.task_name].cl_success = true;
        } else if (event.event_type == EventType::MergeRequestClosed && event.time > table[event.student_name][event.task_name].merge_last_try && event.time <= score_time) {
        ...
        } else if (event.event_type == EventType::MergeRequestOpen && event.time > table[event.student_name][event.task_name].merge_last_try && event.time <= score_time) {
            table[event.student_name][event.task_name].merge_last_try = event.time;
